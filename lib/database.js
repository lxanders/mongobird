...
function closeByConnectionString(connectionString) {
    var connection = connections[connectionString];

    if (connection) {
        return Promise.fromNode(connection.close.bind(connection))
            .tap(function () {
...
function closeAll() {
    var connectionStrings = _.keys(connections);

    return Promise.all(_.map(connectionStrings, closeByConnectionString));
}
...
function getDb(connectionInformation, dbName) {
    ...
    db.close = closeByConnectionString.bind(null, connectionString);
